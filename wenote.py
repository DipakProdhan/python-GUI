...
win = tk.Tk()
win.geometry('1200x800') # use for full window
win.title('Wenote')
...
color_icons = (light_default_icon, dark_icon, red_icon, monokai_icon, night_blue_icon)

color_dict = {         # text     background
...
# font_box = ttk.Combobox(tool_bar, width=30, textvariable=font_family_var)
...
def change_font_color():
    color_var = tk.colorchooser.askcolor()
                                     #   R    G    B
    # print(color_var)               #((160, 160, 160), '#a0a0a0')
                                     #       [0]            [1]   
    text_editor.configure(fg=color_var[1])     #fg = foreground
...
#inside the file menu->command and functinality
#global variable
url = ''
...
#inside the edit menu-> command and functinality
#find functinality
...
edit_menu.add_command(label='Delete', image=delete_icon, compound=tk.LEFT, accelerator='Del', command= lambda:text_editor.delete(1.0, tk.END))
edit_menu.add_separator()  #my
edit_menu.add_command(label='Find', image=find_icon, compound=tk.LEFT, accelerator='Ctrl+F', command= find_func)

#inside the view menu-> checkbutton and functinality
...
#color theme radiobutton and functinality
def change_theme():
...
count = 0
for i in color_dict:
    color_theme_menu.add_radiobutton(label=i, image=color_icons[count], variable=theme_choice, compound=tk.LEFT, command=change_theme)
    count += 1
